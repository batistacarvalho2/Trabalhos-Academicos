        ...
        try{
            ...
            if(tabela.itemExiste(chave)){
                if(tabela.getIten(chave).arquivoExiste(caminho)){//arquivo já existe
                    ...
                    help.getDocumentoPalavra(caminho)->incrementaNumeroVezesPalavra(); //incrementa numero de vezes da palavra no arquivo
...
/* trata palavra converte maiuscula pra minuscula
   valida para se contem maior ou igual a duas letras*/
...
//configurada para ordem crescente
int compare(Retorno p1, Retorno p2){
...
// limpa palavras e arquivos indexados
...
string Interface::buscarTermo(string termo,bool op){//bool caso true pelo menos uma palavra//caso false pelo todas
    ...
    buscaToPalavra(termo); //retorna a chave
    ...
    if(NumRetorno==0){
        striRetorno += "\nNao existe nelhum dos termos que procuras!!\n";
        return striRetorno;
    }
    ...
    //fazer somatorio da string com nome dos arquivos e pronto
...
void Interface::buscaToPalavra(string termo){
    ...
    for(i=0;i<NumRetorno;i++)retorno[i].setChave(string("777")); //caso a chave esteja vazia
...
//calcula relevancia das palavras
void Interface::calcularRelevancia(){
    int i,j;
    float rele; //peso da chave
    for(i=0;i<numArquivosEnBusca;i++){
        rele=0;
        int fij,dj;
