...
void Tabelahash<KEY,GIVEN>::setIten(KEY chave,GIVEN dado){
    ...
    if(this->itemExiste(chave)){
        posi = buscarItem(chave); // busca item verifica se naquela posicao existe uma item excluido
        delete tabela[posi]; // se ja existe a mesma chave ele exclui e insere uma nova e incrementa contador de palavra
        tabela[posi] = new NoDoc<KEY,GIVEN>(chave,dado);
        return;
    }
    while(estaOcupado(posi) && i < n){// se contem dado diferente ele faz uma nova sondagem
        posi = hash2(chave,i++) ;
    }
    if(estaOcupado(posi)){ // caso tabela cheia lança uma
        throw(00);//tabela cheia
    }else{ // caso posicao apos a sondagem estiver vazia insere nova chave
        tabela[posi] = new NoDoc<KEY,GIVEN>(chave,dado);
        elementos++; //incrementa numero de palavra
        cont+=i-1;
    }
}
...
GIVEN Tabelahash<KEY,GIVEN>::getIten(KEY chave){
    if(size()== 0)throw(01); //tabela vazia lança uma exerção
    int posi = this->buscarItem(chave); //busca posicao verifica palavra
